/**
 * Test Case ID: TC64517
 * Description: Reminders Create 3 hour reminder
 * Migrated from: muuk-tests/Frame_Module/TestSteps_2960c0b8.spec.ts
 */
...
test('TC64517 – Reminders: Create 3 hour reminder', async ({ page }) => {
  ...
  // Fill reminder text and select 3-hour option
  await framePage.fillReminderText('Reminder in 3 hours');
  await framePage.clickReminderTime('In 3hrs');

  // Verify reminder appears in the list
  await expect(page.locator("//P[contains(text(),'Reminder in 3 hours')]")).toBeVisible({ timeout: 30000 });
